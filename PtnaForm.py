...
""" グローバル変数の定義
"""
entry = None                            # 入力オブジェクトのオブジェクトを保持
response_area = None            # 応答オブジェクトのオブジェクトを保持
...
log = []                                    # インプット文字列を保持
question = 0                            # 天気予報で使用するフラグ
weather = WeatherResponder()  # WeatherResponderオブジェクトを保持
...
def talk():
    ...
    # questionフラグが1なら入力された地域の天気予報を答える
    elif question == 1:
        # WeatherResponderクラスのis_weather()を実行
...
def run():
    ...
    # イメージを用意
    ptyna_images.append(tk.PhotoImage(file = "wi-fi.png"))
    ptyna_images.append(tk.PhotoImage(file = "wi-fi2.png"))
    ptyna_images.append(tk.PhotoImage(file = "wi-fi3.png"))
    ptyna_images.append(tk.PhotoImage(file = "wi-fi4.png"))
    
    # キャンバス上にイメージを配置
    on_canvas = canvas.create_image(
        0,                                                              # x座標
        0,                                                              # y座標
        image = ptyna_images[0],                                              # 配置するイメージオブジェクトを指定
